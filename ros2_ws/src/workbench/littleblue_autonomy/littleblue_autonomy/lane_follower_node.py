...
class LaneFollowerNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('center_weight', 1.5)
        ...
        self._started = False
        # Course-forward direction, tracked in world frame. Computed from
        # PCA of accumulated lane cells and low-pass filtered so short-term
        # noise (detection gaps, obstacle occlusions) can't flip it. Used
        # to detect when the robot has rotated far off-course so we can
        # rotate-in-place to realign before the A* gets confused by its
        # own body-X-aligned bin ladder being sideways to the course.
        self._lane_forward_angle = None
...
# ─── clearance computation ─────────────────────────────────────
    ...
    def _update_lane_forward_angle(self):
        """Track the world-frame course direction by PCA on recent lane cells.

        The dominant eigenvector of lane-cell positions (in world frame)
        gives the lane axis. Its sign (forward vs. backward) is ambiguous,
        so on first lock we disambiguate with the robot's current heading;
        thereafter we keep the same sign across updates so the tracked
        direction never flips 180° even if the robot turns around.

        Only lane cells within a 15m window of the robot are used, so the
        direction reflects the *current* leg of the course, not a distant
        earlier leg.
        """
        if not self._world_pose_valid:
            return
        world_cells = self._lane_world
        if not world_cells:
            return
        ...
        R2 = 15.0 ** 2
        nearby = [(cwx, cwy)
                  for (cwx, cwy, _) in world_cells.values()
                  if (cwx - wx) ** 2 + (cwy - wy) ** 2 < R2]
        if len(nearby) < 20:
            return

        arr = np.asarray(nearby, dtype=float)
        centered = arr - arr.mean(axis=0)
        cov = (centered.T @ centered) / len(arr)
        _ev, evec = np.linalg.eigh(cov)
        d = evec[:, -1]
        new_angle = math.atan2(float(d[1]), float(d[0]))

        # Sign disambiguation
        if self._lane_forward_angle is None:
            ref = self.world_yaw
        else:
            ref = self._lane_forward_angle
        diff = (new_angle - ref + math.pi) % (2 * math.pi) - math.pi
        if abs(diff) > math.pi / 2.0:
            new_angle = math.atan2(float(-d[1]), float(-d[0]))

        # Low-pass filter so the tracked direction is stable against
        # noise but still follows slow curves in the course.
        if self._lane_forward_angle is None:
            self._lane_forward_angle = new_angle
        else:
            alpha = 0.1
            d_ang = ((new_angle - self._lane_forward_angle + math.pi)
                     % (2 * math.pi) - math.pi)
            self._lane_forward_angle += alpha * d_ang
    ...
    def _smooth_path(self, path, obstacles, lane_boundaries=None, min_x=0.5, bin_w=0.25):
        ...
        for _ in range(n_passes):
            ...
            for i in range(1, len(dense) - 1):
                # Uniform 3-point moving average: equal weighting of the
                # point and its two neighbours. Damps A* bin-choice noise
                # more aggressively than the previous (0.15, 0.7, 0.15)
                # weighted blend.
                sx = (dense[i - 1][0] + dense[i][0] + dense[i + 1][0]) / 3.0
                sy = (dense[i - 1][1] + dense[i][1] + dense[i + 1][1]) / 3.0
    ...
    def _plan_path_legacy(self):
        ...
        for i in range(n_bins):
            ...
            # Lane walls also block waypoint gaps. Without this the gap
            # finder happily places waypoints in the ~0.1m slot between a
            # barrel and a lane line because it only knew about the barrel.
            for ox, oy, er in lane_walls:
                if abs(ox - x_center) >= (max_clr * curve_scale) + bin_w:
                    continue
                clr = self._compute_obstacle_clearance(er, obs_x=ox) * curve_scale
                lo = max(nav_right, oy - clr)
